...
def get_support_points(start, stop, nr_points, func, offset=.1, method='slope', **kwargs):
    """
    Creates a set of support points that are distributed to maximize sampling
    around regions of interest.

    The overall idea is to distribute the sampling points to obtain an equidistance
    spacing by some critertion. Choosing 'slope' as an example, the returned sampling points
    are weighted with the respective slope of the function. 
    ...
    offset: 'float'
        Level of offset added to any background region of un-interest (meaning if we filter for 'curvature', any region without curvature is of un-interest).
        If set to 0, no sample points can be found in the regions without interest.
    
    method: 'str'
        Possible strings are ['slope','curvature','max','equidistant']. The method 
        defines the region of interest where the sample points are generated.
    ...
    if method in ['slope','curvature','max','equidistant']:
        ...
        print("get_support_points can not be operated as Method is not properly defined. The choices are 'slope','curvature' or 'max'.")
        return None
