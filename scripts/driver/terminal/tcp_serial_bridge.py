...
Contact information (as well as this script) lives at
...
def cleanup_for_serial(text):
    ...
    """
    # chr(255) is the "we are negotiating" leading bit.  If it is the first bit
    # in a packet, we do not want to send it on to the serial port
    if ord(text[:1]) == 255:
        return ""
...
class Handler:
    def __init__(self):
        ...
        print("TCP to Serial bridge is up: telnet to localhost:"
              "%s to access %s." % (LISTEN, com.portstr))
...
def main(argv=None):
    ...
    "Parse our arguments"
    try:
        options, args = getopt(argv[1:], "p:r:s:y:b:t:f:l:h", [
            "port=", "baudrate=", "bytesize=", "parity=", "stopbits=",
            "timeout=", "flow=", "listen=", "help"
        ])
    except GetoptError:
        usage()
        return
